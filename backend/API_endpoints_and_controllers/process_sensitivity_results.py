...
2. Executes CFA.py on each modified configuration
    ...
    for config_file in config_files:
        try:
            ...
            if result.returncode == 0:
                success_count += 1
                logger.info(f"Successfully ran CFA for module {module_num}")
            else:
                error_count += 1
                logger.error(f"Error running CFA for module {module_num}: {result.stderr}")
