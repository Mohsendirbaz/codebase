# Add to Calculations_and_Sensitivity.py

# Import the new event sequencing modules
from sensitivity_orchestrator import SensitivityOrchestrator, EventState
from sensitivity_api import sensitivity_bp, init_orchestrator

# Initialize and register handlers
def initialize_sensitivity_orchestrator():
    """Initialize and configure the sensitivity orchestration system."""
    sensitivity_logger = logging.getLogger('sensitivity')
    sensitivity_logger.info("Initializing sensitivity orchestration system")

    # Initialize orchestrator with handlers
    orchestrator = init_orchestrator(BASE_DIR, sensitivity_logger)

    # Register handlers for each state
    orchestrator.register_handler(EventState.CONFIGURED, configure_handler)
    orchestrator.register_handler(EventState.CONFIG_COPIED, copy_config_handler)
    orchestrator.register_handler(EventState.BASELINE_COMPLETED, baseline_handler)
    orchestrator.register_handler(EventState.VARIATIONS_PROCESSED, variations_handler)
    orchestrator.register_handler(EventState.RESULTS_GENERATED, results_handler)
    orchestrator.register_handler(EventState.VISUALIZATIONS_CREATED, visualizations_handler)

    sensitivity_logger.info("Sensitivity orchestration system initialized with all handlers")
    return orchestrator

# Update Flask app initialization
def initialize_app_with_orchestrator():
    """Initialize Flask app with sensitivity orchestration system."""
    app = Flask(__name__)
    CORS(app)
    setup_logging()

    # Initialize orchestrator
    orchestrator = initialize_sensitivity_orchestrator()

    # Register sensitivity blueprint for sequential operations
...
def configure_sensitivity():
    ...
    try:
        ...
        config = {
            ...
            'selectedR': data.get('selectedR', {f'R{i+1}': 'off' for i in range(10)}),
            'selectedRF': data.get('selectedRF', {f'RF{i+1}': 'off' for i in range(5)}),
    ...
    try:
        ...
        result = subprocess.run(
            [
                ...
                json.dumps(config['selectedR']),
                json.dumps(config['selectedRF']),
...
def inject_names():
    """Inject display names and variation labels by suffix-matching only the numeric part of Sxx."""
    sensitivity_logger = logging.getLogger('sensitivity')
    try:
        payload = request.get_json()
        version = payload.get('selectedVersions', [])[0]
        SenParameters = payload.get('SenParameters', {})

        if not version:
            return jsonify({"error": "Missing version"}), 400

        base_dir = os.path.join(BASE_DIR, 'backend', 'Original')
        reports_dir = os.path.join(base_dir, f'Batch({version})', f'Results({version})', 'Sensitivity', 'Reports')
        calsen_file = os.path.join(reports_dir, "calsen_paths.json")

        if not os.path.exists(calsen_file):
            return jsonify({"error": f"{calsen_file} not found"}), 404

        with open(calsen_file, 'r') as f:
            data = json.load(f)
        ...
        for param_id, param_config in SenParameters.items():
            ...
            numeric_suffix = param_id[1:]  # e.g., "34" from "S34"
            matching_key = next((k for k in property_mapping if k.endswith(numeric_suffix)), None)
            display_name = property_mapping.get(matching_key, f"Parameter {numeric_suffix}")
            ...
            for val in values:
                var_str = f"{val:+.2f}"
                mod_val = f"{val:.2f}"  # Can be adjusted to match formatting spec
                label = f"{display_name} ({var_str}, {mod_val})"
                variation_labels[var_str] = label
                var_strs.append(var_str)

            parameter_names[param_id] = {
                "display_name": f"{display_name} ({', '.join(var_strs)})",
                "base_name": display_name,
        ...
        with open(calsen_file, 'w') as f:
            json.dump(data, f, indent=2)

        sensitivity_logger.info(f"Injected {len(parameter_names)} presentable names (numeric suffix match)")
        return jsonify({"status": "success", "parameter_names": parameter_names})

    except Exception as e:
        sensitivity_logger.exception("inject_names failed")
        return jsonify({"error": str(e)}), 500



# This file is meant to be incorporated into Calculations_and_Sensitivity.py, not run directly
# The app initialization and handlers should be defined in Calculations_and_Sensitivity.py

# If this file is run directly, it will not work due to missing handler functions
if __name__ == '__main__':
    print("This file is not meant to be run directly. It should be incorporated into Calculations_and_Sensitivity.py.")
    print("Please run Calculations_and_Sensitivity.py instead, which uses port 2500.")
