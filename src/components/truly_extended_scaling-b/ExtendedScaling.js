...
const ExtendedScaling = ({
  ...
  }, [scalingGroups, baseCosts, addToHistory, propagateChanges, onScalingGroupsChange, determineInsertionIndex]);
  ...
  const exportConfiguration = useCallback(() => {
    ...
    try {
      ...
      const exportData = {
        version: "1.1.0", // Update version to indicate cumulative support
        metadata: {
    ...
    try {
      ...
      reader.onload = (e) => {
        try {
          ...
          } else if (importedData.currentState) {
            // Handle new format
  ...
  }, [scalingGroups, protectedTabs, calculateScaledValue, onScaledValuesChange]);
