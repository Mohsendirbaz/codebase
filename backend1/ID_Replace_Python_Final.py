...
TARGET_PHRASES = [
    "interval_list2", 
    r"C:\Users\md8w7\OneDrive University of Missouri\Desktop\ImportantFiles\Milestone4", 
    "12.64555555"
]  # List of phrases to search for
REPLACEMENT_PHRASES = [
    "Incentive", 
    r"C:\\Users\\Mohse\\OneDrive\\Documents\\GitHub\\TeaSpace", 
    "NewNewNewNewKeyAmount3"
]  # Replacement phrases
...
SPECIFIC = True  # Enable specific file matching
SPECIFIC_TARGET_FILES = ["Front_Subtab_HTML.py", "Front_Subtab_Plot.py"]  # Specific files to search
...
def log_matches(matches, total_files_processed, unique_file_count):
    ...
    total_summary = (
        ...
        f"Unique Files Containing Matches: {unique_file_count}\n"
    )
...
def search_and_replace(directory, target_phrases, replacement_phrases):
    ...
    return matches, total_files_processed, len(files_with_matches)

def main():
    """Main function to initialize log, perform search/replace, and log results."""
    initialize_log()
    matches, total_files_processed, unique_file_count = search_and_replace(
        DESTINATION_DIRECTORY, TARGET_PHRASES, REPLACEMENT_PHRASES
    )
    log_matches(matches, total_files_processed, unique_file_count)
