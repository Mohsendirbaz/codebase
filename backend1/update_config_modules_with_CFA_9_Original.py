...
def calculate_revenue_and_expenses_from_modules(config_received, config_matrix_df, results_folder, version, selected_v, selected_f, price, target_row, iteration):
    ...
    for idx, row in config_matrix_df.iterrows():
        ...
        feedstock_cost_operational[idx] = config_module['rawmaterialAmount3'] * length
        labor_cost_operational[idx] = config_module['laborAmount3'] * length
        utility_cost_operational[idx] = config_module['utilityAmount3'] * length
        maintenance_cost_operational[idx] = config_module['maintenanceAmount3'] * length
        insurance_cost_operational[idx] = config_module['insuranceAmount3'] * length
    
        # Store module selling price for each row
        module_selling_price_operational[idx] = price * length
   ...
   # Create pie chart
    ...
    # Annotate labels with arrows pointing outward
    ...
    ax.set_title('Operational Cost Breakdown', fontsize=14, pad=25)
    ax.set_facecolor('#f7f7f7')
    plt.tight_layout()

    # Save the chart as PNG
    png_path = os.path.join(results_folder, f"Operational_Cost_Breakdown_Pie_Chart({version}).png")
...
def main(version, selected_v, selected_f, target_row):
    ...
    while npv < TOLERANCE_LOWER or npv > TOLERANCE_UPPER:
        ...
        if npv < 0:
            ...
            price *= 0.98  # Decrement price by 2%
