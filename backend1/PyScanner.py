...
DESTINATION_DIRECTORY = r"C:\Users\Mohse\OneDrive\Documents\GitHub\TeaSpace\backend"
TARGET_PHRASES = ["common_utils", "Certitude", "12.6455"]
EXCLUDED_PHRASES = ["PyScanner", "ID_Replace_Python", "ID_Replace_Python_Final"]
SKIP_FOLDERS = ["__pycache__", "venv", ".git"]

# === New Configuration: Specific targets ===
SPECIFIC = False
SPECIFIC_TARGET_FILES = ["Front_Subtab_HTML.py", "Front_Subtab_Plot.py"]
...
def log_matches(matches, total_files_searched):
    """Write matches to the log file in tabular format and log file counts."""
    unique_files = {match['file'] for match in matches}
    ...
    with open(LOG_FILE_PATH, 'a', encoding='utf-8') as log_file:
        ...
        log_file.write(f"\n=== Search Statistics ===\n")
        log_file.write(f"Total files searched: {total_files_searched}\n")
        log_file.write(f"Files containing matches: {unique_file_count}\n")
        log_file.write(f"Match percentage: {(unique_file_count/total_files_searched)*100:.2f}% of searched files\n")
...
def scan_directory(directory, target_phrases, specific_files=None):
    ...
    return matches, total_files_searched

def main():
    """Main function to initialize log, perform scan, and log results."""
    initialize_log()
    matches, total_files_searched = scan_directory(
        DESTINATION_DIRECTORY, 
        TARGET_PHRASES, 
        SPECIFIC_TARGET_FILES if SPECIFIC else None
    )
    log_matches(matches, total_files_searched)
