...
log_file_path = r"C:\Users\md8w7\OneDrive University of Missouri\Desktop\ImportantFiles\Milestone4\backend\PNG.log"
logging.basicConfig(
...
for i, selected_version in enumerate(selected_versions):
    ...
    header = version_headers  # Assign directly since it's a single string now
...
for selected_version in selected_versions:
    version_specific_dir = os.path.join(uploads_dir, f'Batch({selected_version})', f'Results({selected_version})', f'{versions_identifier}_AnnotatedStaticPlots')
